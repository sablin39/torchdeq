...
def register_norm(norm_type, norm_class):
    ...
    Args:
        norm_type (str): The type of normalization to register.
        norm_class (type): The class defining the normalization.
...
def register_norm_module(module_class, norm_type, names='weight', dims=0):
    """
    Registers a to-be-normed module for the user-defined normalization class.

    Args:
        module_class (type): Module class to be indexed.
        norm_type (str): The normalization class to register for.
        names (str, optional): Attribute name for normalization. Default ``'weight'``.
        dims (int, optional): Dimension for norm computation. Default 0.
...
def _is_skip_prefix(name, prefix_filter_out):
    """
    Helper: check if module name starts with any prefix in the filter list.
    """
...
def _is_skip_name(name, filter_out):
    """
    Helper: check if module name contains any string in the filter list.
    """
...
def apply_norm(model, norm_type='weight_norm', prefix_filter_out=None, filter_out=None, args=None, **norm_kwargs):
    ...
    Args:
        ...
        norm_type (str, optional): Type of normalization. Default ``'weight_norm'``.
        prefix_filter_out (list or str, optional): Module name prefixes to skip. Default None.
        filter_out (list or str, optional): Module name substrings to skip. Default None.
    ...
    """
    config = DEQConfig.from_args(args, norm_type=norm_type, **norm_kwargs)

    norm_type = config.norm_type
    if norm_type == 'none':
        return

    if norm_type not in _norm_class:
        raise KeyError(f"Unknown norm type '{norm_type}'. Registered: {list(_norm_class.keys())}")
    ...
    for name, module in model.named_modules():
        ...
        if type(module) in Norm._target_modules:
            module._deq_norm = Norm.apply(module, deq_args=config, **norm_kwargs)
